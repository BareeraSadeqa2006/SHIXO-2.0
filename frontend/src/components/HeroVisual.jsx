...
// Single-path India silhouette (potrace output, MIT — github.com/djaiss/mapsicon).
// Rendered inside a transform group to map its coordinate space into a 0..1024 viewBox.
const INDIA_PATH =
...
// Network nodes (approx city positions in the 0..1024 viewBox) + links — tuned visually.
const NODES = [
  { x: 470, y: 250 }, // north
  { x: 360, y: 560 }, // west
  { x: 645, y: 470 }, // east
  { x: 470, y: 615 }, // centre-south
  { x: 560, y: 720 }, // south-east
  { x: 450, y: 720 }, // south
  { x: 470, y: 820 }, // tip
];
const LINKS = [
  [0, 1], [0, 2], [0, 3], [1, 3], [2, 3],
  [3, 4], [3, 5], [4, 5], [5, 6], [1, 5], [2, 4],
];
...
export default function HeroVisual() {
  const [ref, shown] = useCardReveal();

  return (
    <div ref={ref} className="relative w-full max-w-xl mx-auto aspect-square select-none">
      {/* Ambient light — soft glows, not a container */}
      <div className="pointer-events-none absolute -inset-10 -z-10">
        <div className="absolute left-[18%] top-[14%] h-2/3 w-2/3 rounded-full bg-teal/15 blur-3xl" />
      ...
      >
        <div className="flex items-center gap-3 bg-white/90 backdrop-blur-md rounded-2xl border border-white ring-1 ring-light-gray shadow-[0_20px_44px_-20px_rgba(15,23,42,0.30)] px-4 py-3">
          <span className="w-10 h-10 rounded-xl bg-light-teal text-teal flex items-center justify-center">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </span>
          <div>
            <div className="text-lg font-extrabold text-navy leading-none">95%+</div>
            <div className="text-[11px] font-medium text-slate mt-1">AI Accuracy</div>
          </div>
        </div>
      </div>

      <div
        className={`float-card float-card-2 absolute -right-2 top-1/3 sm:right-0 transition-all duration-700 delay-150 ${
          shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
        }`}
      >
        <div className="flex items-center gap-3 bg-white/90 backdrop-blur-md rounded-2xl border border-white ring-1 ring-light-gray shadow-[0_20px_44px_-20px_rgba(15,23,42,0.30)] px-4 py-3">
          <span className="w-10 h-10 rounded-xl bg-teal/10 text-teal flex items-center justify-center">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.25 18L9 11.25l3.75 3.75L21.75 6M21.75 6h-4.5m4.5 0v4.5" />
            </svg>
          </span>
          <div>
            <div className="text-lg font-extrabold text-navy leading-none">10,000+</div>
            <div className="text-[11px] font-medium text-slate mt-1">Teachers Tracked</div>
          </div>
        </div>
      </div>

      <div
        className={`float-card float-card-3 absolute left-4 bottom-6 sm:left-8 sm:bottom-10 transition-all duration-700 delay-300 ${
          shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
        }`}
      >
        <div className="flex items-center gap-3 bg-white/90 backdrop-blur-md rounded-2xl border border-white ring-1 ring-light-gray shadow-[0_20px_44px_-20px_rgba(15,23,42,0.30)] px-4 py-3">
          <span className="relative flex h-2.5 w-2.5">
            <span className="absolute inline-flex h-full w-full rounded-full bg-success opacity-70 hv-ping" />
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-success" />
          </span>
          <div className="flex items-end gap-1 h-7">
            <span className="w-1.5 rounded-sm bg-teal/40" style={{ height: '40%' }} />
            <span className="w-1.5 rounded-sm bg-teal/60" style={{ height: '70%' }} />
            <span className="w-1.5 rounded-sm bg-teal" style={{ height: '100%' }} />
            <span className="w-1.5 rounded-sm bg-teal/70" style={{ height: '55%' }} />
          </div>
          <div>
            <div className="text-lg font-extrabold text-navy leading-none">20</div>
            <div className="text-[11px] font-medium text-slate mt-1">Mandals Live</div>
          </div>
        </div>
      </div>
    </div>
