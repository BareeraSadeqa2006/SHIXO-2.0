...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-soft-white">
      {/* Header - White with subtle shadow and border */}
      <header className="sticky top-0 z-50 bg-white border-b border-light-gray shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <img src="/favicon2.jpeg" alt="SHIXO Logo" className="w-10 h-10 rounded-lg object-cover shadow-md" />
              <div>
                <h1 className="text-xl font-bold tracking-wide text-navy">SHIXO</h1>
                <p className="text-xs text-light-gray opacity-80">Government Education Portal</p>
              </div>
            </div>
            <nav className="hidden md:flex items-center gap-6">
              <a href="#features" className="text-sm text-navy hover:text-navy-light transition-colors font-bold">Features</a>
              <a href="#analytics" className="text-sm text-navy hover:text-navy-light transition-colors font-bold">Analytics</a>
              <a href="#about" className="text-sm text-navy hover:text-navy-light transition-colors font-bold">About</a>
              <Link to="/login" className="bg-navy hover:bg-navy-light text-white px-5 py-2 rounded text-sm font-medium transition-colors shadow-sm">
                Login Portal
              </Link>
            </nav>
            <Link to="/login" className="md:hidden bg-navy hover:bg-navy-light text-white px-4 py-2 rounded text-sm font-medium">
              Login
            </Link>
          </div>
        </div>
      ...
      {/* Hero - Light background with clear separation */}
      <section className="bg-soft-white pt-12 pb-20 lg:pb-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 bg-navy/5 rounded-full px-4 py-1.5 mb-6">
                <span className="w-2 h-2 bg-success rounded-full"></span>
                <span className="text-xs font-medium text-navy">AI-Powered Governance Platform</span>
              </div>
              <h2 className="text-4xl lg:text-5xl font-bold leading-tight mb-6 text-navy">
              ...
              <div className="flex flex-wrap gap-4">
                <Link to="/login" className="bg-navy hover:bg-navy-light text-white px-8 py-3 rounded-lg font-semibold text-sm transition-colors shadow-md">
                  Access Portal
                </Link>
                <a href="#features" className="border-2 border-navy text-navy hover:bg-navy hover:text-white px-8 py-3 rounded-lg font-medium text-sm transition-colors">
                  Explore Features
                </a>
              </div>
            </div>
            <div className="hidden lg:flex justify-center items-center">
              <img 
                ...
                alt="Teacher Illustration - Female teacher with blue book, India map background, government building"
                className="w-full h-300 max-w-sm h-auto object-contain drop-shadow-lg"
              />
            </div>
          </div>
        </div>
